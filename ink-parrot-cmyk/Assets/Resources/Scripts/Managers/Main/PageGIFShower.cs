...
public class PageGIFShower : MonoBehaviour
{
    ...
    {
        isPlaying = false;
        GifSize = GifImages.Length;

        BaseAspect = GifImages[0].rect.width / GifImages[0].rect.height;

        if (TargetImage != null)
        {
            RectTransform rt = TargetImage.rectTransform;

            // Inspector에서 지정한 최초 크기 저장
    ...
    void ChangeImage(int index)
{
    ...
    else
    {
        ...
        float currentAspect =
            GifImages[index].rect.width / GifImages[index].rect.height;

        // 첫 프레임 대비 비율
        float ratio = currentAspect / BaseAspect;

        // Inspector에서 지정한 최초 크기를 기준으로 폭 계산
        ...
        rt.SetSizeWithCurrentAnchors(
            ...
            BaseHeight);
            Debug.Log(
    $"BaseWidth={BaseWidth}, BaseAspect={BaseAspect}, CurrentAspect={currentAspect}, Ratio={ratio}, Width={width}");
    }
}
}
